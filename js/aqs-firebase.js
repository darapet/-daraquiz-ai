...
async function actionPublishQuiz(data) {
    var id = data.id || data.quiz_id;
    if (!id) throw new Error('Quiz ID required.');
    /* Build public URL — detect Capacitor/native vs web hosting */
    var base;
    if (typeof window !== 'undefined' && window.location) {
        var proto = window.location.protocol;
        if (proto === 'capacitor:' || proto === 'file:' || proto === 'ionic:' || proto === 'app:') {
            /* Running inside Capacitor mobile app — use the known live domain */
            base = 'https://darapet.github.io/-daraquiz-ai/';
        } else {
            /* Web browser — derive dynamically so it works on any host/path */
            base = window.location.origin + window.location.pathname.replace(/\/[^\/]*$/, '/');
        }
    } else {
        base = 'https://darapet.github.io/-daraquiz-ai/';
    }
    var quizUrl      = base + 'take-quiz.html?id=' + id;
    var challengeUrl = base + 'challenge.html?quiz=' + id;
    /* Store the canonical URLs in Firestore so they can be fetched later */
    ...
    return { success: true, id: id, quiz_url: quizUrl, challenge_url: challengeUrl };
}
