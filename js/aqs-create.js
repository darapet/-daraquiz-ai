...
(function ($) {
    ...
    function normalizeMath(text) {
        ...
        /* \( ... \) → $ ... $ (inline) */
        text = text.replace(/\\\(([\s\S]+?)\\\)/g, function(_, m) { return '$' + m.trim() + '$'; });
        /* \begin{equation/align} ... \end{...} → $$ ... $$ */
        text = text.replace(/\\begin\{(?:equation|align)\*?\}([\s\S]+?)\\end\{(?:equation|align)\*?\}/g, function(_, m) { return '$$' + m.trim() + '$$'; });
        /* Bare LaTeX commands without $ wrappers — wrap them so KaTeX renders */
        /* e.g. \frac{1}{2}, \sqrt{x}, \int_0^1 not already inside $ $ */
        text = text.replace(/(^|[^$])(\\(?:frac|sqrt|int|sum|prod|lim|infty|partial|alpha|beta|gamma|delta|theta|pi|sigma|omega|lambda|mu|nu|epsilon|phi|psi|xi|eta|tau|rho|chi|kappa|iota|zeta|times|div|pm|cdot|leq|geq|neq|approx|equiv|rightarrow|leftarrow|forall|exists|nabla|hbar|mathbb|mathrm|text)(?:\{[^$
]*?\})*(?:\{[^$
]*?\})?(?:\^[{]?[^$
]*?[}]?)?(?:_[{]?[^$
]*?[}]?)?)([^$]|$)/g, function(m, pre, latex, post) {
    ...
    $('#aqs-publish-btn').on('click', function() {
        ...
        $.post(AQS.ajax_url, {
            ...
            $.post(AQS.ajax_url, pubPayload, function(pubRes) {
                ...
                if (pubRes.success) {
                    /* Use URLs from server; fallback built from live domain */
                    var _proto = window.location.protocol;
                    var _base  = (_proto === 'capacitor:' || _proto === 'file:' || _proto === 'ionic:' || _proto === 'app:')
                        ? 'https://darapet.github.io/-daraquiz-ai/'
                        : window.location.origin + window.location.pathname.replace(/\/[^\/]*$/, '/');
                    var quizUrl       = pubRes.data.quiz_url       || (_base + 'take-quiz.html?id='    + currentQuizId);
                    var challengeUrl  = pubRes.data.challenge_url  || (_base + 'challenge.html?quiz='  + currentQuizId);
    ...
    /* ══════════════════════════════════════════════════════════
       COPY TOAST — shows a brief slide-up notification on copy
    ══════════════════════════════════════════════════════════ */
    function showCopyToast(msg) {
        var toast = document.getElementById('aqs-copy-toast');
        if (!toast) return;
    ...
    $(document).on('click', '#aqs-copy-quiz-link', function() {
        var url = $('#aqs-quiz-link').val();
        if (!url) return;
        navigator.clipboard.writeText(url).then(function() {
            showCopyToast('✅ Quiz link copied!');
            var $b = $('#aqs-copy-quiz-link');
            $b.html('✅ Copied!');
            setTimeout(function() { $b.html('📋 Copy'); }, 2000);
        }).catch(function() {
            /* Fallback for older Android WebViews */
            $('#aqs-quiz-link').select();
            document.execCommand('copy');
            showCopyToast('✅ Quiz link copied!');
        });
    });
    $(document).on('click', '#aqs-copy-challenge-link', function() {
        var url = $('#aqs-challenge-link').val();
        if (!url) return;
        navigator.clipboard.writeText(url).then(function() {
            showCopyToast('✅ Challenge link copied!');
            var $b = $('#aqs-copy-challenge-link');
            $b.html('✅ Copied!');
            setTimeout(function() { $b.html('📋 Copy'); }, 2000);
        }).catch(function() {
            $('#aqs-challenge-link').select();
            document.execCommand('copy');
            showCopyToast('✅ Challenge link copied!');
        });
    });
    ...
    function openQrModal(url, title, subtitle) {
        var modal   = document.getElementById('aqs-qr-modal');
        var canvas  = document.getElementById('aqs-qr-canvas');
        var dlBtn   = document.getElementById('aqs-qr-download');
        var urlText = document.getElementById('aqs-qr-url-text');
        var modalTitle = document.getElementById('aqs-qr-modal-title');
        var modalSub   = document.getElementById('aqs-qr-modal-sub');
        ...
        if (modalTitle) modalTitle.textContent = title || 'Scan to Open';
        if (modalSub)   modalSub.textContent   = subtitle || 'Point your phone camera at the QR code';
        ...
        if (typeof QRCode !== 'undefined') {
            ...
            /* Fallback: Google Charts API QR */
            var img = document.createElement('img');
            img.src = 'https://chart.googleapis.com/chart?cht=qr&chs=210x210&chl=' + encodeURIComponent(url) + '&choe=UTF-8';
    ...
    /* QR button triggers */
    $(document).on('click', '#aqs-qr-quiz-link', function() {
        var url = $('#aqs-quiz-link').val();
        if (!url) return;
        openQrModal(url, '🎓 Quiz QR Code', 'Scan to take the quiz on any phone');
    });
    $(document).on('click', '#aqs-qr-challenge-link', function() {
        var url = $('#aqs-challenge-link').val();
        if (!url) return;
        openQrModal(url, '⚡ Challenge QR Code', 'Scan to join the live challenge');
    });

    /* Close QR modal */
    $(document).on('click', '#aqs-qr-modal-close, #aqs-qr-modal', function(e) {
