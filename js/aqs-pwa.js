...
(function () {
    ...
    var CHANGELOG = [
        {
            ...
            date:    'May 2026',
            changes: [
                '\U0001f527 Buttons fixed on all pages — Firebase module fully rebuilt',
                '\U0001f5bc Hugging Face AI replaces Pollinations for image generation',
                '\U0001f4e3 News ticker slowed down for comfortable reading',
                '\U0001f4f1 Hardware back button now navigates back instead of closing app',
                '🤖 All AI powered by Groq — faster responses across every feature',
                '\U0001f509 Text-to-speech rebuilt — Groq voices + Pollinations fallback',
                '\U0001f510 Admin-only panel added for easy app management'
            ]
        },
        {
            version: 'v2',
    ...
    /* ══════════════════════════════════════════════════════
       ...
       ══════════════════════════════════════════════════════ */
    /* ══════════════════════════════════════════════════════
       CURRENT APP VERSION — bump this every time you push
       Must match the version entry at the TOP of CHANGELOG
       ══════════════════════════════════════════════════════ */
    var CURRENT_VERSION = CHANGELOG.length ? CHANGELOG[0].version : 'v1';

    /* Live domain always used for version checks (works in Capacitor too) */
    var LIVE_BASE = 'https://darapet.github.io/-daraquiz-ai/';

    /* ══════════════════════════════════════════════════════
       SERVICE WORKER — Web browser only
       SW file must be at the ROOT of the web app (www/),
       NOT inside www/js/ — required for correct scope.
       ══════════════════════════════════════════════════════ */
    if ('serviceWorker' in navigator) {
        window.addEventListener('load', function () {
            /* Derive the absolute path to aqs-sw.js at the site root */
            var swUrl = window.location.pathname.replace(/\/[^/]*$/, '/') + 'aqs-sw.js';

            navigator.serviceWorker.register(swUrl, { scope: './' })
                .then(function (reg) {
                    console.log('[XZILY PWA] SW registered at:', swUrl, '| scope:', reg.scope);
                ...
                .catch(function (err) {
                    console.warn('[XZILY PWA] SW registration failed:', err);
                    /* SW failed (e.g. Capacitor) — fall back to version.json check */
                    checkVersionJson();
                });
    ...
    /* ══════════════════════════════════════════════════════
       CAPACITOR / NATIVE APP UPDATE CHECK
       Capacitor WebViews don't support service workers.
       We poll version.json from the live GitHub Pages site
       and show the same banner when a new version is found.
       ══════════════════════════════════════════════════════ */
    function checkVersionJson() {
        /* Only run when we're inside a Capacitor / file: context */
        var proto = window.location.protocol;
        var isNative = (proto === 'capacitor:' || proto === 'file:' ||
                        proto === 'ionic:' || proto === 'app:');
        if (!isNative) return;

        /* Cache-bust to always get the latest file */
        var url = LIVE_BASE + 'version.json?t=' + Date.now();
        fetch(url, { cache: 'no-store' })
            .then(function (r) { return r.ok ? r.json() : null; })
            .then(function (data) {
                if (!data || !data.version) return;
                var stored = '';
                try { stored = localStorage.getItem('_aqsInstalledVersion') || ''; } catch (e) {}
                /* First run — just store the version, don't annoy the user */
                if (!stored) {
                    try { localStorage.setItem('_aqsInstalledVersion', CURRENT_VERSION); } catch (e) {}
                    return;
                }
                /* New version available */
                if (data.version !== CURRENT_VERSION) {
                    showNativeUpdateBanner(data.version, data.update_url || LIVE_BASE);
                }
            })
            .catch(function () { /* offline or blocked — silently ignore */ });
    }

    function showNativeUpdateBanner(newVersion, updateUrl) {
        ...
        banner.innerHTML =
            ...
            '<span><strong>Update ' + newVersion + ' available!</strong> Open the browser to get the latest version.</span>' +
            '<button id="aqs-update-now-btn" class="aqs-upd-btn-primary">Open</button>' +
        ...
        document.getElementById('aqs-update-now-btn').addEventListener('click', function () {
            banner.remove();
            /* Try Capacitor Browser plugin; fall back to window.open */
    ...
    /* Kick off the native check on page load (after a short delay so
       it doesn't block anything critical) */
    window.addEventListener('load', function () {
        setTimeout(checkVersionJson, 4000);
    });
