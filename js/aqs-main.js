...
(function ($) {
    ...
    if ($('#format-single').length) { /* create-quiz v2: handled by aqs-create.js */
        ...
        $('#aqs-publish-btn').on('click', function () {
            ...
            $.post(AQS.ajax_url, saveQuizPayload(qs), function (res) {
                ...
                $.post(AQS.ajax_url, pubPayload, function (pubRes) {
                    ...
                    /* ── Quiz take link ── */
                    var _proto2 = window.location.protocol;
                    var _base = (_proto2 === 'capacitor:' || _proto2 === 'file:' || _proto2 === 'ionic:' || _proto2 === 'app:')
                        ? 'https://darapet.github.io/-daraquiz-ai/'
                        : window.location.origin + window.location.pathname.replace(/\/[^\/]*$/, '/');
                    $('#aqs-quiz-link').val(pubRes.data.quiz_url || (_base + 'take-quiz.html?id=' + currentQuizId));
        ...
        /* ── Toast helper (reuse if already defined by aqs-create.js) ── */
        ...
        $(document).on('click', '#aqs-copy-link', function () {
            ...
            navigator.clipboard.writeText(val).then(function () {
                _showCopyToast('✅ Quiz link copied!');
                $('#aqs-copy-link').html('✅ Copied!');
                setTimeout(function () { $('#aqs-copy-link').html('📋 Copy'); }, 2000);
            }).catch(function () {
        ...
        $(document).on('click', '#aqs-copy-challenge-link', function () {
            ...
            if (!val) return;
            navigator.clipboard.writeText(val).then(function () {
                _showCopyToast('✅ Challenge link copied!');
                $('#aqs-copy-challenge-link').html('✅ Copied!');
                setTimeout(function () { $('#aqs-copy-challenge-link').html('📋 Copy'); }, 2000);
            }).catch(function () {
        ...
        /* ── QR Code popup (also handles buttons added by create-quiz.html) ── */
        function _openQrModal(url, title, sub) {
            if (typeof openQrModal === 'function') { openQrModal(url, title, sub); return; }
            /* Fallback if QRCode library isn't loaded — open URL in new tab */
            window.open('https://chart.googleapis.com/chart?cht=qr&chs=300x300&chl=' + encodeURIComponent(url), '_blank');
        }
        $(document).on('click', '#aqs-qr-quiz-link', function () {
            var url = $('#aqs-quiz-link').val(); if (!url) return;
            _openQrModal(url, '🎓 Quiz QR Code', 'Scan to take the quiz on any phone');
        });
        $(document).on('click', '#aqs-qr-challenge-link', function () {
            var url = $('#aqs-challenge-link').val(); if (!url) return;
            _openQrModal(url, '⚡ Challenge QR Code', 'Scan to join the live challenge');
        });
    }
