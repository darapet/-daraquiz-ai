...
(function () {
    ...
    async function hfGenerateImage(prompt, width, height, seed) {
        ...
        for (var mi = 0; mi < HF_MODELS.length; mi++) {
            try {
                ...
                var tid  = setTimeout(function () { ctrl.abort(); }, 180000); /* 3 min — HF models cold-start slowly */
    ...
    /* ── Pollinations AI — free, no auth, reliable fallback ── */
    async function pollinationsGenerateImage(prompt, width, height, seed) {
        var w = Math.min(width  || 1024, 1280);
        var h = Math.min(height || 1024, 1280);
        ...
        var ctrl = new AbortController();
        var tid  = setTimeout(function () { ctrl.abort(); }, 180000); /* 3 min — same as HF */
        try {
            var res = await fetch(url, { signal: ctrl.signal });
            clearTimeout(tid);
            if (!res.ok) throw new Error('Pollinations HTTP ' + res.status);
            var blob = await res.blob();
            if (!blob || blob.size < 500) throw new Error('Pollinations returned empty image');
            return { url: URL.createObjectURL(blob), isPollinations: true, blob: blob };
        } catch (e) { clearTimeout(tid); throw e; }
    }

    async function raceImage(prompt, width, height, seed, statusCallback) {
        if (statusCallback) statusCallback('Generating image via AI… (may take up to 60 s)');
        /* 1. Try Hugging Face first */
        try {
            return await hfGenerateImage(prompt, width, height, seed);
        } catch (hfErr) {
            console.warn('[ImageGen] HF failed, trying Pollinations:', hfErr.message);
        }
        /* 2. Free fallback — Pollinations (flux model, no auth needed) */
        if (statusCallback) statusCallback('Switching to backup AI server… (please wait)');
        try {
            return await pollinationsGenerateImage(prompt, width, height, seed);
        } catch (polErr) {
            throw new Error('Image generation failed on all servers. Check your internet connection and try again.');
        }
    }
    ...
    async function callAI(messages) {
        ...
        try {
            var ctrl = new AbortController();
            var tid  = setTimeout(function () { ctrl.abort(); }, 30000); /* 30s for model status check */
    ...
    function showError(msg) {
        ...
        setTimeout(function () { $error.style.display = 'none'; }, 12000);
    }
